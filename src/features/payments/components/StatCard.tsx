...
export default function StatCard({ title, value, showInfo = false }: StatCardProps) {
  return (
    <div className="min-w-70 flex-1 rounded-2xl border border-gray-200 bg-white p-4 sm:min-w-0">
      <div className="flex flex-col gap-1">
        <div className="flex items-center justify-between">
          <h3 className="text-h6 text-kalto-black">{value}</h3>
          {showInfo && <InfoIcon className="text-kalto-black" />}
        </div>
        <p className="font-aeonik text-body-sm text-kalto-black-green-400">{title}</p>
      </div>
    </div>
  );
}
